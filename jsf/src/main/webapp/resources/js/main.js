jQuery(document).ready(function($){
	...
	itemInfoWrapper.each(function(){
        ...
        // showSlides(container, sliderPagination);//TEST
...
function nextSlide(){
    ...
    var wrapDiv =$('.cd-single-item.cd-slider-active')

	if(wrapDiv.length > 0){
    	return;
	}

	container =$('.cd-slider-wrapper');
    pagination = $('.cd-slider-wrapper').find('.cd-slider-pagination li');
	...
	n =  container.find('.cd-slider-pagination .selected a').html();
        if(n == pagination.size()){
        	n =1;
            prevSlide(container, pagination , n);
            return;
		}
